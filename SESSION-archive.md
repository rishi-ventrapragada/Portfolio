...
Handoff notes for increments 30 and older, moved out of SESSION.md in
increment 32 so the live file stays short. Newest at the top. The standing
constraints and the two latest increments are in SESSION.md.
...
handoff. Each built clean, then pushed, aliased and re-measured
...
built clean (0 errors / warnings / hints), pushed, aliased and re-measured on
...
- **Deployed and aliased.** Git integration built
  ...
  auto-assigned production domain and to the local `dist` (no
...
- **Deployed and aliased.** Git integration built
  ...
  `vercel alias set` moved `rishi-ventrapragada.vercel.app` to it, and the
  vanity HTML is byte-identical (sha1) to the deployment, to the
  auto-assigned production domain and to the local `dist` (two
...
- **Deployed and aliased.** Git integration built
  the new deployment (Ready, 17s);
  `vercel alias set` moved `rishi-ventrapragada.vercel.app` to it, and the
  vanity HTML is byte-identical (sha1) to the deployment, to the
  auto-assigned production domain and to the local `dist`. Both
...
- Console clean on every run. Screenshots reviewed: desktop × four states
  ...
  web and KalaCart.
- **Deployed and aliased.** Git integration built
  the new deployment (Ready, 21s);
  `vercel alias set` moved `rishi-ventrapragada.vercel.app` to it, and the
  vanity HTML is byte-identical (sha1) to the deployment, to the
  auto-assigned production domain and to the local `dist`. The
...
- **Deployed and aliased.** Git integration built
  the new deployment (Ready 27s after the
  ...
  auto-assigned production domain. The same harness run against the
...
one project**, and only three in the whole
...
(`Age: 8684`), while the project's auto-assigned `*.vercel.app` domain serves the
...
account** (none of its projects is this portfolio), and there is no Vercel CLI auth in this environment.
...
the project's auto-assigned `*.vercel.app` domain served the current build. Fixed with
`vercel alias set <deployment> rishi-ventrapragada.vercel.app`.

**There is only one Vercel project.** It was renamed `portfolio` →
`rishi-ventrapragada`. `vercel ls` prints each
...
- Live (public): the project's auto-assigned `*.vercel.app` domain
...
- **Deployment Protection is ON** (`ssoProtection: all_except_custom_domains`), so every
  ...
  original alias the project's auto-assigned `*.vercel.app` domain predates it and is public.
