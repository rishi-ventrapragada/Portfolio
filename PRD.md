...
**Twinkle** is an opacity keyframe with a per-star negative `animation-delay`, so every star is already mid-cycle on the first frame; near and mid only, far is a still backdrop. **Bloom** is a static `box-shadow` on the 2px and 3px stars only — depth, not animation, so it costs nothing per frame. **Three meteors** on pairwise-coprime periods (23s, 31s, 37s), each visible for ~18% of its own period, so their starts coincide only once every lcm = 26 381s.
