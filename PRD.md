...
A section between the "currently building" strip and the footer (`#experience`, increment 13; visual rebuild in increment 13.1): eyebrow "Journey" and h2 "Experience" at shell width, then an NLE-style editor that runs **viewport edge to edge** — a black **monitor** above a multi-track **timeline**. `ExperienceTimeline.astro` owns the section, the moments data and the monitor box; `ExperienceFrame.astro` renders one monitor frame; `ExperienceTrack.astro` renders the ruler, both track rows and the playhead; `ExperienceClip.astro` is one clip button; `src/scripts/experience-timeline.ts` holds commit / preview behaviour and `src/scripts/experience-playhead.ts` the playhead.
...
**Monitor.** Full viewport width, `height: 60vh; min-height: 340px`, `overflow: hidden`, no padding, border or radius, on `--monitor-bg` — the site's one pure-black surface (§4.2), chosen so it reads as a screen rather than a card. Every frame is in the HTML, stacked in the same grid cell (`grid-area: 1 / 1`), so a swap is a cross-fade inside a box whose height never changes: 540px at 1280×900 and 400.19px at 375×667 in every state. A frame centres its content (`place-items: center`, `2rem var(--gutter) 3rem` padding): `.meta` year, `<h3>` keyword at `clamp(1.75rem, 4vw, 3rem)`, the body at `clamp(1.125rem, 2.2vw, 1.5rem)` and at most 44ch. The KalaCart frame splits 2fr / 3fr from 768px with the cover on the right; the cover is at most 560px wide and `30vh` tall, so it never overflows the box (491 × 270 rendered at 1280×900). A decorative `aria-hidden` timecode (`00:00:00:00`, `:15`, `:30`, `:45` — cosmetic, not a clock) sits bottom-right of each frame. Inactive frames are `opacity: 0; visibility: hidden`, so only the active one is in the accessibility tree. A swap is a 180ms opacity cross-fade (`visibility` flips after the fade on the way out, at once on the way in); under `prefers-reduced-motion: reduce` there is no transition and the swap is instant. The first frame is active from the server, so the section reads without JavaScript.

**Timeline.** Full viewport width on `--timeline-bg`, `overflow-x: auto`. Three rows, each `header | lane`: the header is a 90px sticky-left cell on `--bg-raised` with a 1px `--line` right edge; every lane is the same `repeat(4, minmax(150px, 1fr))` grid with 2px gaps, so ruler ticks, clips and waveforms line up by construction (tick left = clip left, 0px off, at both widths). Below 694px (90 + 4 × 150 + gaps) the rows overflow and the timeline scrolls sideways under the sticky headers; the document never does.

- **Ruler** (`aria-hidden`): blank corner, then one tick cell per clip column with a 1px `--line` left edge, minor ticks every quarter column along the bottom, and a `.meta` timecode.
- **V1 / video**: the four clips in a `role="group"` (`aria-label="Moments"`). A clip is a real `<button type="button">` styled as a solid block: `--tone` (one of `--word-amber` / `--word-sky` / `--word-mint` / `--word-periwinkle`, in that order) ground, `--bg` text, a 1px `--timeline-bg` edge, 2px radius, a `repeating-linear-gradient` diagonal texture in `currentColor` at 0.14 opacity, `.meta` year top-left and the keyword bottom-left in `--font-display` 700 at 0.875rem, minimum 4.5rem tall. Hover → `brightness(1.08)`; pressed (`aria-pressed="true"`) → an inset 2px `--accent` ring; the focus ring is the global `:focus-visible`. The accessible name is the two visible labels (year + keyword) and `aria-describedby` points at the frame's body sentence.
- **A1 / notes** (`aria-hidden`): one cell per column with an inline SVG polyline in `--tone` at 0.35 opacity. Decoration only: no controls, no tab stops (0 focusable nodes measured).
- **Playhead** (`aria-hidden`): a 2px `--accent` line spanning all three rows with a triangle flag in the ruler, absolutely positioned inside the rows. Its CSS default puts it on the first clip's left edge (header + lane padding + 1px) so the no-JS page is right; the script only ever sets `transform: translateX()` from the active clip's `getBoundingClientRect()` relative to the rows (0px off the clip's left edge in all four states at 1280 and 375), re-measured by a `ResizeObserver` on the rows and the V1 lane. The move is a 200ms transform transition; `0s` under reduced motion.

**Planned clip.** The Recurzn clip carries the shared `.planned` utility from `global.css` (increment 13): `opacity: 0.5` plus `border-style: dashed`, which shows as a dashed dark edge on the dimmed periwinkle block. The clip's own defaults sit in `@layer components`, so the unlayered utility wins without `!important`; any future planned element takes the same class. `ProjectCard.astro` still uses its own `data-status="planned"` rule (solid border); moving it onto `.planned` is a separate change.

**Behaviour** (`experience-timeline.ts`). Click and keyboard focus **commit** a clip: it becomes the pressed one, its frame shows and the playhead moves to it. On fine pointers only (`(hover: hover) and (pointer: fine)`), hovering a clip **previews** its frame — the playhead follows the preview too — without changing the pressed clip, and leaving the track restores the committed frame. **A preview needs a real cursor movement**: there is no `mouseenter` handler; the lane listens to `mousemove` and previews only when the event's coordinates differ from the last ones seen (or, for the very first event, `movementX/Y` is non-zero) *and* lie inside the target clip's current rect. Scrolling the page under a stationary cursor fires `mouseenter` on whatever passes beneath it — Chrome fired exactly one in the regression run — and previously looked like the clips cycling on their own; it now changes nothing. **No `aria-live`**, on purpose: the clip's name and description already say exactly what the monitor shows. Enter and Space activate the button natively — the script handles no keys.

Verified in increment 13.1 (SESSION.md, headless Chrome over CDP at 1280×900 and 375×667): playhead on the active clip's left edge in all four states at both widths; ruler ticks 0px off their clips; monitor height constant; monitor `left 0`, width = viewport, no horizontal page overflow; A1 row has no focusable node and Tab lands on the four clips then the footer; Enter and Space commit; scroll-under-cursor changes nothing, a real move previews, leaving reverts; reduced motion `0s` on frame and playhead; no-JS first frame visible with the playhead on it; both accents; image loads (560 × 308 natural); console clean. Client JS **3585 B gzipped, all inline** (3272 B at increment 13), against the 40 KB cap.
