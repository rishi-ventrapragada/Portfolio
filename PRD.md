...
A section between the Tech stack and Projects (`#experience`; increment 13, visual rebuild 13.1, scroll scrub 14): eyebrow "Journey" and h2 "Experience" at shell width, then an NLE-style editor that runs **viewport edge to edge** — a black **monitor** above a multi-track **timeline** — **pinned while the page scrolls through it, so scrolling scrubs the timeline**. `ExperienceTimeline.astro` owns the section, the pin, the moments data and the monitor box; `ExperienceFrame.astro` renders one monitor frame; `ExperienceTrack.astro` renders the ruler, both track rows and the playhead; `ExperienceClip.astro` is one clip button; `src/scripts/experience-timeline.ts` holds the one `commit` path plus click / focus / hover, `src/scripts/experience-scrub.ts` maps scroll to it, and `src/scripts/experience-playhead.ts` moves the playhead.
...
**Interaction — three equal ways to pick a clip, one code path.** Scrolling through the pinned section, clicking a clip, or focusing it from the keyboard (Tab, then Enter / Space) all call the same `commit(id)`: that clip becomes `aria-pressed="true"`, its frame shows in the monitor and the playhead moves to it. None is secondary: a click or a key press wins at once at any scroll position, and the next scroll *zone change* takes over again.

- **Scroll scrub** (`experience-scrub.ts`). The section is a `--pin-length: 250vh` scroll track (the one number to tune; each clip zone is a quarter of the pinned distance, i.e. 37.5vh) and its inner `.pin` is `position: sticky; top: 0; height: 100dvh` with `padding-top: var(--nav-height)` — the `Wordmark.astro` technique. Progress is `(scrollY − sectionTop) / (sectionHeight − innerHeight)` clamped 0–1, computed from the absolute scroll position each frame (stateless, so scrolling back reverses exactly), and the zone is `min(3, floor(progress × 4))`; edges measured at exactly 25 / 50 / 75 % at 1280×900 and 375×667. Only a zone change commits. The `scroll` and `resize` listeners are passive and only schedule one `requestAnimationFrame`; the frame reads and calls `commit` — no per-event layout work (CLAUDE.md §4) — and an `IntersectionObserver` attaches them only while the section is on screen. Past the track the pin releases and the page scrolls on into Projects. The `#experience` anchor lands on the pin start (`scroll-margin-top: 0`; the pin pads for the nav itself), so the nav link shows the first clip.
- **Click / keyboard.** Real `<button>`s; Enter and Space are native activation, the script handles no keys. Focus commits too, so Tab alone previews nothing — it commits.
...
**Reduced motion.** The pin stays active: `position: sticky` is layout, not animation, and a zone change is a discrete state change, so the mechanic adds no motion beyond the user's own scroll. What it triggers obeys the existing rules — the cross-fade and the playhead glide are `0s` under `prefers-reduced-motion: reduce`, measured after a scroll-driven commit. The alternative (un-pin the section under reduce, leaving click / keyboard only) was considered and rejected in increment 14: it would remove the section's primary interaction for those users without removing any motion.

**No JavaScript.** A `<noscript><style>` inside the section (the BootPreloader pattern) makes the section `height: auto`, the pin `position: static` and the monitor `60vh / min 340px`, so the section is an ordinary content-height block showing the first clip with the playhead on it, and the page scrolls past it into Projects. With JavaScript the pinned CSS is the default, so nothing shifts at hydration.

Verified in increment 14 (SESSION.md, headless Chrome over CDP at 1280×900 and 375×667, local then live): scrolling the track in 5 % steps commits `web → python → kalacart → recurzn` forward and the reverse backward; pin `top` 0 throughout, release past the track with Projects entering from below; a wheel event advances; a click on Recurzn from zone 0 commits at once and survives a same-zone scroll; hover on a scroll-committed clip previews and `mouseleave` returns to that clip; focus, Enter and Space commit at 5 / 40 / 95 % progress; no-JS un-pinned; no horizontal overflow, section height exactly 250vh. Client JS **3829 B gzipped, all inline** (3585 B at 13.1), against the 40 KB cap.
