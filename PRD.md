...
| Tech stack: a skill tree of the dev-side skills (a video-editing constellation is a separate section, next) | `#skills` | §5.6 |
...
**Tech stack (`#skills`) — a skill tree (increment 18).** Eyebrow "Toolkit", h2 "Tech stack" (`SkillTree.astro`; one `SkillGroup.astro` per category, its leaves in `SkillChain.astro`; `TechStack.astro` deleted, `Pills.astro` kept for the project panels). **This is the dev/build side only.** A second, separate Skills section — a constellation map of the video-editing skills — is the next increment: it is its own section with its own anchor and must not be merged into, or confused with, this tree. **Content, verbatim** (six category roots, then a learning cluster): Languages (C, Python, Java, HTML, CSS, JavaScript, TypeScript, Dart); Frameworks (Flutter, Astro, React, Next.js, Node.js, Django, FastAPI, Tailwind CSS, Vite); AI (Claude, Claude Code, Gemini, GPT-6 Astra, Codex, Ollama, Open Router); Tools (Notion, Figma — Canva moved to the video section); Platforms (Supabase, Firebase, Vercel, Render); Cloud (Google Cloud, AWS); Currently learning (Docker, Kubernetes). 41 nodes. Nothing links anywhere; every node is static text and its visible text is its accessible name (no per-skill detail exists to reveal, so no hover or focus state). Outline: h2 → one `<h3>` per root; leaves are `<li>`s of one `<ul>` per category.
...
**From 960px, a talent tree**: a six-column grid at shell width, two columns per group; bands fall out of the flow — Languages, Frameworks, AI; then Tools, Platforms, Cloud; then the learning cluster placed on the middle pair. Each group is root → a 28px **fan** (an SVG `viewBox="0 0 2 1"`, `preserveAspectRatio="none"`, two lines from the root's bottom centre to each column's top centre, `vector-effect="non-scaling-stroke"` so they stay 2px at any width) → the leaves in two columns, filled top to bottom then across (`grid-auto-flow: column`, `--rows: ceil(n/2)` from the build: 4/4, 5/4, 4/3, 1/1, 2/2, 1/1, 1/1), each leaf joined to the one above by a 16px vertical connector; the head of a column has none, the fan reaches it. Six 128px pills plus gaps and gutters need 944px, hence 960. Measured: 181px leaves at 1280, 126–130px at 960, no overflow, groups 379px wide in three aligned bands.
...
17. `[done]` Increment 18: Tech stack rebuilt as an RPG-style skill tree — six solid-accent roots fanning to 14px leaf pills in two columns, a dashed `.planned` learning cluster, stacked spine-and-chain layout below 960px, no script; `TechStack.astro` deleted (§5.6). The video-editing constellation is the next, separate section.
