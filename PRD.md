...
--sprocket-hole  the surface a perforation reads through to   var(--bg) in both
--shadow         the colour a card's drop shadow mixes from
```

`--on-accent` is not `--bg`: measured, near-black reads **5.38:1** on dark-theme crimson but only **3.11:1** on the light scope's darkened crimson, where white reads **6.02:1** — the right ink flips with the theme while `--bg` flips the other way. `--sprocket-hole` is not `--bg` because ProjectFrame's holes are *painted*, not punched, so a light-scoped card would have kept painting `#111214` dots. `--shadow` replaces a `color-mix` of `--monitor-bg`, which is the Experience NLE surface being borrowed purely as a source of black, and is far too heavy on a light card. `AboutPanel` carries a fourth, local `--tag-ink`, which flips the *opposite* way from `--on-accent` (see §5.6).
...
**The section is light-scoped** since increment 22 (§5.16). `.shell` moved from the `<section>` to an inner wrapper at the same time: a background on a 1280px-capped element would have left the page's dark ground either side of it on a wide screen. Frame surfaces, sprocket holes and drop shadows all follow the scope through tokens (§4.2).
