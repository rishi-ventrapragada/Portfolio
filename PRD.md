...
Both `[data-past-hero]` and `.is-scrolled` now come off the hero's **real box** via `[data-hero]` and one IntersectionObserver with `rootMargin: 0px 0px -100% 0px`, replacing a synthetic 1×1 sentinel prepended at a hardcoded `top: 100dvh` — which only ever worked because the hero happens to be one viewport tall. The hero carries a data attribute rather than an id, since every `[id]` inherits a 5rem `scroll-margin-top` from `global.css` and nothing links there.
