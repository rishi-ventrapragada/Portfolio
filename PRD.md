...
| Video editing: a drifting constellation map of the video-editing skills | `#skills-video` | §5.6 |
...
| Contact: the footer — staged credits over a topographic backdrop | `#contact` | §5.8, §5.15 |
...
2. Seeds the leaves on a **jittered ring** around the root, radius scaled by leaf count, with a quarter-turn phase offset for groups of three or fewer so a two-item category does not land dead horizontal and read as a row.
...
**No animation.** The tree is static — no reveal, no draw-in — so there is nothing to change under `prefers-reduced-motion` (measured: `animation-name: none` on every element in the section, both layouts). No client script; the scatter is computed in `src/lib/skill-scatter.ts` at build time. The section does sit over a starfield (§5.14), which has its own reduced-motion branch. Nav "Skills" → `#skills` lands the section top at 80px (measured).
